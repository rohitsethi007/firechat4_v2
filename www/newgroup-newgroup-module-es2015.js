...
let NewgroupPage = class NewgroupPage {
    constructor(router, imageProvider, dataProvider, alertCtrl, angularfire, afAuth, loadingProvider, camera, formBuilder) {
        this.router = router;
    ...
    ionViewDidEnter() {
        ...
        this.dataProvider.getCurrentUser().snapshotChanges().subscribe((accountRes) => {
            let account = Object.assign({ $key: accountRes.key }, accountRes.payload.val());
                ...
                for (var i = 0; i < account.friends.length; i++) {
                    this.dataProvider.getUser(account.friends[i]).snapshotChanges().subscribe((friendRes) => {
                        if (friendRes.key != null) {
                            let friend = Object.assign({ $key: friendRes.key }, friendRes.payload.val());
