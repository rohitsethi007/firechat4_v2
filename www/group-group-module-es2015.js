...
let GroupPage = class GroupPage {
    ...
    constructor(dataProvider, navCtrl, modalCtrl, angularfire, alertCtrl, imageProvider, loadingProvider, camera, keyboard, actionSheet, contacts, geolocation, route, router, popoverCtrl) {
    ...
    getGroupDetailsandMessages() {
        ...
        this.subscription = this.dataProvider.getGroup(this.groupId).snapshotChanges().subscribe((group) => {
            if (group.payload.exists()) {
                ...
                if (this.group.members) {
                    this.group.members.forEach((memberId) => {
                        this.dataProvider.getUser(memberId).snapshotChanges().subscribe((member) => {
                            if (member.key != null) {
                                member = Object.assign({ $key: member.key }, member.payload.val());
                ...
                this.dataProvider.getGroupMembers(group.key).snapshotChanges().subscribe((memberIdsRes) => {
    ...
    getPolls() {
        this.subscription = this.dataProvider.getGroupPolls(this.groupId).snapshotChanges().subscribe((pollIdsRes) => {
                ...
                pollIds.forEach((pollId) => {
                    ...
                    if (pId != null && pId !== 'system0000') {
                        this.dataProvider.getPollDetails(pId).snapshotChanges().subscribe((pollRes) => {
                            const poll = Object.assign({ key: pollRes.key }, pollRes.payload.val());
                            console.log(poll);
    ...
    getResources() {
        this.dataProvider.getGroupResources(this.groupId).snapshotChanges().subscribe((resourceIdsRes) => {
                ...
                resourceIds.forEach((resourceId) => {
                    ...
                    if (rId != null && rId != "system0000") {
                        this.dataProvider.getResourceDetails(rId).snapshotChanges().subscribe((resourceRes) => {
                            let resource = Object.assign({ key: resourceRes.key }, resourceRes.payload.val());
                ...
                eventIds.forEach((eventId) => {
                    ...
                    if (eId != null && eId != "system0000") {
                        this.dataProvider.getEventDetails(eId).snapshotChanges().subscribe((eventRes) => {
                            let event = Object.assign({ key: eventRes.key }, eventRes.payload.val());
                            console.log(event);
