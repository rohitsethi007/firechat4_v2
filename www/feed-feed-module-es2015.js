...
let FeedPage = class FeedPage {
    constructor(dataProvider, navCtrl, modalCtrl, firestore, alertCtrl, loadingProvider, actionSheet, route, router) {
    ...
    ionViewDidEnter() {
        ...
        this.groupId = 'SWSn0dRwU3IksNSMGrvn';
        ...
        this.dataProvider.getConversations().get().subscribe((conversationsInfoRes) => {
                    ...
                    if (conversationInfo.blocked != true) {
                        this.dataProvider.getConversation(conversationInfo.conversationId).snapshotChanges().subscribe((conversationRes) => {
                            if (conversationRes.payload.exists) {
                                let conversation = Object.assign({ $key: conversationRes.key }, conversationRes.payload.data());
