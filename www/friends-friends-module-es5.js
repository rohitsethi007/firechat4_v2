      ...
      _createClass(FriendsPage, [{
        ...
        value: function ionViewDidEnter() {
          ...
          if (this.afAuth.auth.currentUser != null) {
            this.dataProvider.getRequests(this.afAuth.auth.currentUser.uid).snapshotChanges().subscribe(function (requestsRes) {
              var requests = requestsRes.payload.val();
              console.log(requests);

              if (requests != null) {
                if (requests.friendRequests != null && requests.friendRequests != undefined) _this.friendRequestCount = requests.friendRequests.length;else _this.friendRequestCount = 0;
              } else _this.friendRequestCount = 0;
          ...
          this.dataProvider.getCurrentUser().snapshotChanges().subscribe(function (account) {
            ...
            if (account.payload.val() != null && account.payload.val().friends != null) {
              for (var i = 0; i < account.payload.val().friends.length; i++) {
                _this2.dataProvider.getUser(account.payload.val().friends[i]).snapshotChanges().subscribe(function (friend) {
                  if (friend.key != null) {
        ...
        value: function getFriendRequests() {
          ...
          this.dataProvider.getCurrentUser().snapshotChanges().subscribe(function (account) {
            _this3.account = account.payload.val();
        ...
        value: function addOrUpdateFriendRequest(sender) {
          if (!this.friendRequests) {
            ...
            if (index > -1) {
              if (!this.isFriends(sender.$key)) this.friendRequests[index] = sender;
            } else {
              if (!this.isFriends(sender.$key)) this.friendRequests.push(sender);
            }
          }
        ...
        value: function addOrUpdateRequestSent(receiver) {
          if (!this.requestsSent) {
            ...
            if (index > -1) {
              if (!this.isFriends(receiver.$key)) this.requestsSent[index] = receiver;
            } else {
              if (!this.isFriends(receiver.$key)) this.requestsSent.push(receiver);
            }
          }
        }
      }, {
        ...
        value: function findNewFriends() {
          ...
          this.dataProvider.getUsers().snapshotChanges().subscribe(function (accounts) {
            ...
            _this4.dataProvider.getCurrentUser().snapshotChanges().subscribe(function (account) {
              // Add own userId as exludedIds.
              // console.log(account.payload.val());
              _this4.excludedIds = [];
              _this4.account = account.payload.val();

              if (_this4.excludedIds.indexOf(account.key) == -1) {
                _this4.excludedIds.push(account.key);
              ...
              _this4.dataProvider.getRequests(account.key).snapshotChanges().subscribe(function (requests) {
                if (requests.payload.val() != null) {
                  _this4.requestsSent = requests.payload.val().requestsSent;
                  _this4.friendRequests = requests.payload.val().friendRequests;
                }
              });
            });
          });
