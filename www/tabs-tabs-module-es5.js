      ...
      _createClass(TabsPage, [{
        ...
        value: function ionViewDidEnter() {
          ...
          this.storage.get('currentUser').then(function (val) {
            ...
            if (val === null) {
              _this.dataProvider.getCurrentUser().snapshotChanges().subscribe(function (account) {
                // set a key/value
                _this.storage.set('currentUser', account.payload.val());
              });
            }
          }); // Get friend requests count.

          this.dataProvider.getRequests(firebase__WEBPACK_IMPORTED_MODULE_4__["auth"]().currentUser.uid).snapshotChanges().subscribe(function (requestsRes) {
          ...
          this.dataProvider.getConversations().snapshotChanges().subscribe(function (conversationsInfoRes) {
            ...
            conversationsInfo = conversationsInfoRes.map(function (c) {
                ...
                if (conversationInfo.blocked != true) {
                  _this.dataProvider.getConversation(conversationInfo.conversationId).snapshotChanges().subscribe(function (conversationRes) {
                    if (conversationRes.payload.exists()) {
                      ...
                      if (conversation.blocked != true) _this.addOrUpdateConversation(conversation);
                    }
                  });
                }
              });
