...
let TabsPage = class TabsPage {
    ...
    ionViewDidEnter() {
        ...
        this.dataProvider.getRequests(firebase__WEBPACK_IMPORTED_MODULE_4__["auth"]().currentUser.uid).snapshotChanges().subscribe((requestsRes) => {
        ...
        this.dataProvider.getConversations().snapshotChanges().subscribe((conversationsInfoRes) => {
            ...
            conversationsInfo = conversationsInfoRes.map(c => (Object.assign({ $key: c.key }, c.payload.val())));
                    ...
                    if (conversationInfo.blocked != true) {
                        this.dataProvider.getConversation(conversationInfo.conversationId).snapshotChanges().subscribe((conversationRes) => {
                            if (conversationRes.payload.exists()) {
                                let conversation = Object.assign({ $key: conversationRes.key }, conversationRes.payload.val());
        ...
        this.dataProvider.getGroups().snapshotChanges().subscribe((groupIdsRes) => {
            let groupIds = [];
            groupIds = groupIdsRes.map(c => (Object.assign({ $key: c.key }, c.payload.val())));
            if (groupIds.length > 0) {
                ...
                groupIds.forEach((groupId) => {
                    this.dataProvider.getGroup(groupId.$key).snapshotChanges().subscribe((groupRes) => {
                        let group = Object.assign({ $key: groupRes.key }, groupRes.payload.val());
                        if (group.$key != null) {
